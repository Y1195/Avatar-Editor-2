...
function init(self)
	...
	maid:Mark(element.InputBegan:Connect(function(input)
		if (VALID_PRESS[input.UserInputType]) then
			...
			local inputEndConn; inputEndConn = input.Changed:Connect(function()
	...
	maid:Mark(element.InputChanged:Connect(function(input)
	...
	maid:Mark(UIS.InputChanged:Connect(function(input, process)
		if input == dragInput and self.IsDragging then
